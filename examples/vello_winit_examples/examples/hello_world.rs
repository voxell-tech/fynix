...
impl FynixDemo for HelloWorld {
    ...
    fn build(&mut self, ctx: &mut FynixCtx<()>) -> ElementId {
        ctx.set(field_accessor!(<EmptyBtn>::corner_radius), 8.0);
            ...
            p.set_child(ctx.add_with::<Vertical>(|v, ctx| {
                ...
                v.add(ctx.add_with::<Label>(|label, _ctx| {
                    label.text = "Lorem ipsum dolor sit amet consectetur \
                        adipiscing elit. Placerat in id cursus mi pretium \
                        tellus duis. Urna tempor pulvinar vivamus fringilla \
                        lacus nec metus. Integer nunc posuere ut hendrerit \
                        semper vel class."
                        .into();
                }));
